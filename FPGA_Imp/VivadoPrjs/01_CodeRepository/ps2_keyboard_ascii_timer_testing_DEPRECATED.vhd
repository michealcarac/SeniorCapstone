...
ENTITY ps2_keyboard_ascii_timer IS
  GENERIC(
  ...
  PORT(
      ...
      ascii_new_test : out std_logic;
      break_o    : OUT STD_LOGIC; -- Break
      break_test : out std_logic;
...
ARCHITECTURE behavior OF ps2_keyboard_ascii_timer IS
  ...
  component timer is
   generic(OUTPUT_WIDTH : Integer;   -- Max Output Width of 32 (Integer Limitations)
           CLOCK_DIVIDER: Integer);  -- Default Divider of 5000, period of 1/10000. If 100MHz clock, this allows for a resolution of 0.0001
   port(
      clk   : in std_logic;
      reset : in std_logic;
      en    : in std_logic;
...
BEGIN
  ...
  ascii_new_test <= ascii_new_prev;
  --int_o   <= '1' when state = done_translate else -- Create an interrupt when done translating, otherwise stay low
  --           '0';
  int_o <= int_reg;
    ...
    IF(clk'EVENT AND clk = '1') THEN
      ...
      CASE state IS
        ...
        WHEN done_translate =>
            ...
            int_reg <= '1';  -- MC -> Set interrupt low. This will be pulsed for twice every key press. Once for press down, once for let go
