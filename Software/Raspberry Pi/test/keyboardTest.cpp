...
int main() {
    ...
    while (!done) {
        XNextEvent(d, &event);
        time = (duration_cast<milliseconds >(system_clock::now().time_since_epoch()).count() - start) / 1000.0f;
        switch (event.type) {
            case KeyPress: {
                ...
                stroke = new Keypress((float) time, false, char(XLookupKeysym(&event.xkey, 0))); // false = pressed
                ...
                if (was_it_auto_repeat(d, &event, KeyRelease, KeyPress)) {
                    ...
                    stroke = new Keypress((float) time, true, char(XLookupKeysym(&event.xkey, 0))); // true = released
                    presses->appendKeypress(*stroke);
